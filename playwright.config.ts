...
const e2eProfile = process.env.PLAYWRIGHT_E2E_PROFILE ?? "local-fast";
const includeManualSuites = process.env.PLAYWRIGHT_INCLUDE_MANUAL === "true";
const localFastWorkers = process.env.PLAYWRIGHT_FAST_WORKERS ?? "50%";
...
export default defineConfig({
  ...
  workers: e2eProfile === "local-fast" ? localFastWorkers : undefined,
  projects: [
    {
      name: "chromium",
      use: {
