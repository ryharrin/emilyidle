...
must_haves:
  truths:
    ...
    - "Locked watch and upgrade cards show an always-on lock reason row"
  artifacts:
...
- Assert the CTA button exists via `data-testid` (eg `next-unlock-cta-career`).
- Assert at least one locked watch card renders a lock reason row via `data-testid` (e.g. `locked-item-hint-classic`).
- Assert at least one locked upgrade renders a lock reason row via `data-testid` (e.g. `locked-upgrade-hint-atelier-tools`).

Test B: "catalog empty state CTA returns to vault"
- Seed a save where Catalog tab is visible:
  - set state so `getUnlockVisibilityRatio(state, "showcase") >= REVEAL_THRESHOLD_RATIO`
  - achieve this by seeding owned items/collection value consistent with the `showcase` milestone requirement
