...
Scope: Phase 22 covers Catalog empty states only; other tab empty states remain unchanged.
...
- Use the existing `onNavigate` prop added in Plan 22-03; only add it here if it is missing.
...
Use minimal props based on `CatalogTab` signature:
- `isActive: true`, `catalogSearch: ""`, `catalogBrand: "all"`, `catalogStyle: "all"`, `catalogSort: "default"`, `catalogEra: "all"`, `catalogType: "all"`, `catalogTab: "unowned"`
- noop handlers for the `onCatalog*Change` callbacks and `onCatalogTabChange`
- `catalogBrands: []`, `filteredCatalogEntries: []`, `discoveredCatalogEntries: []`, `discoveredCatalogIds: []`, `catalogEntries: []`, `hasOwnedCatalogTiers: false`
- `onNavigate: vi.fn()`
  </action>
