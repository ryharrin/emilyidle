...
must_haves:
  truths:
    - "UI can render lock reason rows with correct requirement text + progress for milestones/achievements/prestige"
    - "Progress detail aligns with authoritative milestone/achievement requirements"
  artifacts:
  ...
  key_links:
    - from: "tests/unlock-progress.unit.test.ts"
      to: "src/game/selectors/index.ts"
      via: "unit coverage of progress helpers"
...
- `getPrestigeUnlockProgressDetail(state: GameState, prestigeId: "workshop" | "maison" | "nostalgia"): UnlockProgressDetail`
  - Uses `state.enjoymentCents` as current and the corresponding prestige threshold as `threshold`.
  - Labels should use `getEnjoymentThresholdLabel(threshold)` with a short prefix like "Reach".
  - `ratio` is `current / threshold` clamped to [0, 1].

- Ensure `src/game/state.ts` re-exports the new helpers and types so UI imports can come from the game facade.
...
- Prestige detail helper:
  - workshop/maison/nostalgia return the correct threshold and clamped ratio
