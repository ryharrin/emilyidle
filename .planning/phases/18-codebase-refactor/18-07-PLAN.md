...
must_haves:
  truths:
    ...
    - "Player can load an existing save (or start fresh) and reach the Vault tab without crashes (verified via Playwright)"
  artifacts:
...
<task type="auto">
  <name>Task 1: Run full verification suite and fix regressions</name>
  <files>
src/App.tsx
src/game/state.ts
tests/tabs.spec.ts
...
tests/nostalgia-prestige.spec.ts
  ...
  <action>
- Run the full suite and fix failures without expanding scope:
  - `pnpm run lint`
  - `pnpm run typecheck`
  - `pnpm run test:unit`
  - `pnpm run test:e2e`
  - `pnpm run build`
- If failures are caused by selector drift, prefer restoring selectors; if unavoidable, update tests to match the new structure.
- Do not change save payload shape, save version, or localStorage keys.
  </action>
  <verify>pnpm run lint && pnpm run typecheck && pnpm run test:unit && pnpm run test:e2e && pnpm run build</verify>
  <done>
