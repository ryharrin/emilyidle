...
must_haves:
  truths:
    ...
    - "src/game/model/types.ts stays free of src/game/catalog.ts imports (no import.meta.env leakage into the model layer)"
...
- Keep `src/game/model/types.ts` as a pure model module: do NOT import from `src/game/catalog.ts`.
  - If you need a catalog entry id type, define `CatalogEntryId` locally in `src/game/model/types.ts` (e.g., `export type CatalogEntryId = string`) or relocate that type to a lower-level module (not `catalog.ts`).
  - Goal: avoid pulling `import.meta.env.BASE_URL` and catalog concerns into the model layer.
...
- Do not change any runtime behavior, save payload shape, or localStorage keys.
  ...
  <verify>pnpm run typecheck && test -z "$(rg -n 'from\\s+\"\\.\\./catalog(\\.ts)?\"' src/game/model/types.ts || true)"</verify>
  <done>
...
- `src/game/model/types.ts` does not import from `src/game/catalog.ts`.
