...
must_haves:
  truths:
    - "Each prestige tier (Atelier/Maison/Nostalgia) has an explicit Gain/Keep/Lose summary derived from current action semantics"
    - "Summary copy is data-driven (single definition per tier) so UI cannot drift from game rules"
  artifacts:
