...
Do not change the existing two-step armed confirm pattern; the Cancel button must only disarm (no state changes).
  ...
  <done>Atelier and Maison panels show Gain/Keep/Lose before confirm, and Cancel leaves the game state unchanged.</done>
</task>
...
Do not introduce new persistence; this is UI-only.
  ...
  <verify>pnpm run typecheck</verify>
  <done>Nostalgia prestige confirmation uses the same Gain/Keep/Lose structure as Atelier and Maison.</done>
</task>
