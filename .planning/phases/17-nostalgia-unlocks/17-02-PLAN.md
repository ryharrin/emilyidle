...
must_haves:
  truths:
    - "After first nostalgia prestige, player can see an Unlocks store in the Nostalgia tab with clear disabled states"
    - "Player can buy the next available unlock (classic -> chronograph -> tourbillon) based on strict nostalgia unlock order and see nostalgia points update"
    - "Player can refund the most recent unlock for a full nostalgia refund"
    - "Nostalgia tab stays visible after spending nostalgia points down to 0 (once prestiged or once any unlock exists)"
  artifacts:
    - path: src/App.tsx
...
<objective>
Extend the existing Nostalgia tab with an unlock store UI (shown after first prestige) to buy/refund nostalgia unlocks, with an optional confirmation toggle (default ON).
...
<task type="auto">
  ...
  <action>
In `src/App.tsx`, extend the existing Nostalgia tab and panel logic:

- Fix the Nostalgia tab visibility regression:
  - Update the `showNostalgiaPanel` / `tabVisibility.nostalgia` condition so the Nostalgia tab does NOT disappear after spending nostalgia points to 0.
  - The Nostalgia tab must be visible when either `state.nostalgiaResets > 0` OR `state.nostalgiaUnlockedItems.length > 0` (in addition to the existing conditions).

Then extend the existing Nostalgia panel (the branch where `showNostalgiaPanel` is true) by adding an Unlocks section.

Important UI gating:
- Render the unlock store ONLY after first nostalgia prestige: `state.nostalgiaResets >= 1`.
- Do NOT render the unlock store in the teaser-only state.
...
- Render an unlock store section under the existing prestige UI:
...
- Add stable Vault selectors for Playwright (safe additive test ids):
  - On the Vault tab, add `data-testid={\`vault-buy-${id}\`}` to the single-buy button for each watch card.
  - (Optional) Add `data-testid={\`vault-buy-bulk-${id}\`}` for the bulk buy button.
...
- Disabled button/copy should remain readable in both themes.
  ...
  <verify>pnpm run typecheck</verify>
  <done>
Unlock store is visible after first nostalgia prestige (in the full Nostalgia panel), supports buy + refund flows, and respects confirmation toggle and order/affordability disabled states.
  </done>
</task>
...
<success_criteria>
- Unlock store renders only after first prestige (`state.nostalgiaResets >= 1`) and uses stable new `data-testid` hooks.
